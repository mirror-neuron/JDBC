...
WHERE NAME = '홍길동';
--> 한 줄 구성
SELECT * FROM (SELECT SID, NAME, KOR, ENG, MAT,(KOR+ENG+MAT) AS TOT ,(KOR+ENG+MAT)/3 AS AVG, RANK() OVER(ORDER BY (KOR+ENG+MAT) DESC) AS RANK FROM TBL_SCORE ORDER BY SID ASC)WHERE NAME = '홍길동'
;
--==>> 1	홍길동	90	80	70	240	80	1
